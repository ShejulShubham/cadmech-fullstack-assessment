...
function App() {
  ...
  const fetchAllData = async (activeFilters = filters) => {
    ...
    try {
      ...
      setError(
        "Could not establish synchronization with the server. Please check connection status.",
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleDeleteItem = async (id, name) => {
    if (
      window.confirm(
        `Are you absolutely sure you want to permanently delete "${name}" from the system inventory?`,
      )
    ) {
      try {
        setLoading(true);
        ...
        setError(
          err.message || "Failed to successfully drop target asset profile.",
        );
      } finally {
        setLoading(false);
      }
  ...
  return (
    <div className="app">
      ...
      <main className="app-main">
        {/* TODO: Instead of this create a loading component */}
        {/* Centralized Global Loading State Indicator (Addresses Render Sleep Cycles) */}
        {loading && (
          <div className="global-loader">
            <div className="spinner"></div>
            <p>
              Synchronizing assets... (Waking up server if needed, please wait)
            </p>
          </div>
        )}
